# Формат JSON ArenaTES3JSON 0.4.1
...
## Метка времени файла (0.4.1)

У первого `Header` возможно поле `_arena_file_mtime_utc` — исходное время последнего изменения файла в UTC, например `"2002-05-01T12:34:56.1234567Z"`. Это дополнительное поле самого JSON, не часть `HEDR` TES3. Оно удаляется до десериализации плагина и по умолчанию применяется к выходному `.esm/.esp` **после** завершения записи. Можно указать свою дату в GUI или через `--file-date`; дата в JSON не переписывается при изменении опции вывода. Если `_arena_file_mtime_utc` нет, создаётся файл с текущей датой.

Поле `Header.file_type` остаётся обычным semantic-полем `"Esm"` или `"Esp"`. Опция `--file-type esm|esp` обновляет его перед компоновкой и требует соответствующего расширения выходного файла.
